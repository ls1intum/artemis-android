...
internal class QuizParticipationE2eTest : QuizParticipationBaseE2eTest() {
    ...
    fun `can submit quiz - empty submission`() {
    ...
    @Test(timeout = DefaultTestTimeoutMillis)
    fun `can submit quiz - multiple choice`() {
        testQuizSubmissionImpl(
    ...
    @Test(timeout = DefaultTestTimeoutMillis)
    fun `can submit quiz - short answer`() {
        testQuizSubmissionImpl(
            setupAndVerify = { viewModel, submit ->
                val quizQuestionsWithData = viewModel.quizQuestionsWithData.filterSuccess().first()
                val saData =
    ...
    @Test(timeout = DefaultTestTimeoutMillis)
    fun `can submit quiz - drag and drop`() {
        testQuizSubmissionImpl(
            setupAndVerify = { viewModel, submit ->
                val quizQuestionsWithData = viewModel.quizQuestionsWithData.filterSuccess().first()
                val dndData =
    ...
    private fun testQuizSubmissionImpl(
        ...
        runBlocking {
            withTimeout(DefaultTimeoutMillis) {
                setupAndVerify(viewModel) {
                    ...
                    val results =
                        assertNotNull(participation.results, "Results is null on participation")
                    assertTrue(results.isNotEmpty(), "Results does not contain any element")
                    val submission: QuizSubmission =
                        assertIs(
                            results.first().submission,

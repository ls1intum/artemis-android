        ...
        LazyColumn(
            ...
            items(posts) { post ->
                PostItem(
                    modifier = Modifier.fillMaxWidth(),
                    post = post,
                    onClickReply = { onClickReply(it.clientPostId) },
                    onClickViewReplies = { onClickViewReplies(it.clientPostId) }
                )
            }
        }
    }
...
private fun PostItem(
    modifier: Modifier,
    post: Post?,
    onClickReply: (Post) -> Unit,
    onClickViewReplies: (Post) -> Unit
) {
    Card(modifier = modifier) {
        Column(
            ...
            StandalonePostFooter(
                ...
                replyCount = post?.answerPostings?.size ?: 0,
                onClickReply = { if (post != null) onClickReply(post) },
                onClickViewReplies = { if (post != null) onClickViewReplies(post) }
            )
        }
    }
}
...
private fun StandalonePostFooter(
    ...
    Row(modifier = modifier.placeholder(isPlaceholder)) {
        ...
        if (replyCount > 0) {
            TextButton(onClick = onClickViewReplies) {
                Text(
